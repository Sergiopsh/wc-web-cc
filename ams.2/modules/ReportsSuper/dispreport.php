...
<?=$strReports?>: Общая статистика Диспетчеров
</div>
<?
$iformat=dt_format();
...
if ($duration1&&$duration2) $date_clause.=" AND ratesec >= $duration1 AND ratesec < $duration2";
    ...
    $QUERY = "SELECT COUNT(IF(verb = 'ENTERQUEUE',1,NULL)) AS all_s,
                     ...
                     FROM queue_log WHERE queue = 'dispetcher' ";
...
<?
   if ($dateperiod<20){
    $QUERY = "SELECT users1.last_name,
              ...
              AVG(IF(queue = 'dispetcher' AND verb LIKE 'COMPLETE%',data2,NULL)) AS asr_talk,
              SUM(IF(queue = 'dispetcher' AND verb LIKE 'COMPLETE%',data2,0)) AS all_talk,
              queue_log.agent
              FROM queue_log
