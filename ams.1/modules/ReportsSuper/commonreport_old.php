...
if ($duration1&&$duration2) $date_clause.=" AND ratesec >= $duration1 AND ratesec < $duration2";
    ...
    $QUERY = "SELECT COUNT(IF(verb = 'ENTERQUEUE',1,NULL)) AS all_calls,
                     COUNT(IF(verb = 'ABANDON' AND data3 > 5,1,NULL)) AS noanswer,
                     ...
                     FROM queue_log WHERE queue = 'callcenterqueue' ";
...
<?
  ...
  if ($dateperiod<20){
    ...
    $QUERY = "SELECT HOUR(FROM_UNIXTIME(time_id)) AS HR,
	      COUNT(IF(verb = 'ENTERQUEUE',1,NULL)) AS all_calls,
	      COUNT(IF(verb = 'ABANDON' AND data3 > 5,1,NULL)) AS noanswer
	      FROM queue_log
	      WHERE queue = 'callcenterqueue' ";
    $QUERY.=$date_clause;
    $QUERY.=" GROUP BY HR;";
...
<?
   if ($dateperiod<20){
    $QUERY = "SELECT users1.last_name,
              ...
              AVG(IF(queue = 'callcenterqueue' AND verb LIKE 'COMPLETE%',data2,NULL)) AS asr_talk,
              SUM(IF(queue = 'callcenterqueue' AND verb LIKE 'COMPLETE%',data2,0)) AS all_talk,
              queue_log.agent
              FROM queue_log
              LEFT JOIN (SELECT CONCAT('Agent/',TRIM(User_name)) AS Agent, last_name
              FROM users GROUP BY user_name) AS users1
              ON queue_log.agent=users1.agent
              WHERE queue_log.agent LIKE '%14%' ";
    $QUERY.=$date_clause;
...
?>
    ...
    <IMG SRC="modules/ReportsSuper/images/<?=$agents_res?>" WIDTH="48%" ALIGN=RIGHT>

    <table border="0" cellspacing="0" cellpadding="0" width="50%" class="report-tabl">
    <tr>
    <td>
    <table border="0" cellspacing="1" cellpadding="1" width="100%">
	<tr id="head">
