...
echo "</form>";
   ...
   $query = "SELECT calldate, src, dst, billsec, UNIX_TIMESTAMP(calldate) AS time_id, uniqueid
...
//$tbl->tblHead(array("2","","","","","","",""),array("$dh","№ телефона","Время ожидания","Продолжительность","Оператор","Статус","Положил трубку"),"100%");
	...
	for($j=$a; $j<$b; $j++){
		...
		if (!$cheked) $f_name = "";
	?>
	<td width=6><a title="<?=$strDownloadFile?>"  href="http://10.10.11.34/records/search.php?&find="><img src="images/download.gif" ></a></td>	
	<td align="center" nowrap><?=$call_list[$j][0];?></td>
	<td align="center" nowrap><?=$call_list[$j][1];?></td>
