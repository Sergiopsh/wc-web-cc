    ...
    */
    ...
    if (is_array(current($array))) {
      ...
      while (list(,$val) = each($array)) {
        $val   = $this->_valToCsvHelper($val, $separator, $trimFunction);
	//if($export_csv_charset)
	if(function_exists('iconv') && !empty($client_charset) && $client_charset != "UTF-8")
          $ret[] = iconv("UTF-8",$client_charset,$val);
	else $ret[]=$val;  
      }
...
/*	$mime_type="application/ms-excel; charset=windows-1251";	       */
        return $mime_type;
    }
    ...
    */
    function dump($query_string, $filename="dump", $ext="csv", $dbname="mysql", $user="root", $password="", $host="localhost",$db_type="postgres")
    {
		 ...
		 //header('Content-Type: ' ."windows-1251");
                 header('Expires: ' . $now);
