...
<?=$strReports?>: Общая статистика Агента
</div>
<?
$iformat=dt_format();
...
$p->action="loadModule(1,'','operator_reports','CommonReport',\$H({search: 1}));"; $p->align2="right";
...
$date_clause=" time_id >= $start_time AND time_id < $end_time";
if ($duration1&&$duration2) $date_clause.=" AND ratesec >= $duration1 AND ratesec < $duration2";
    ...
    $QUERY = "SELECT COUNT(IF(verb = 'CONNECT' and agent like '%$agent%',1,NULL)) AS all_calls,
                     COUNT(IF(verb = 'ABANDON',1,NULL)) AS noanswer,
                     AVG(IF(verb LIKE 'COMPLETE%' and agent like '%$agent%',data1,NULL)) AS wait,
                     AVG(IF(verb LIKE 'COMPLETE%' and agent like '%$agent%',data2,NULL)) AS avg_calls,
                     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 > 120 and agent like '%$agent%',1,NULL)) AS over_two,
					 COUNT(IF(verb = 'TRANSFER',1,NULL)) AS trnsfered
					 FROM queuemetrics.queue_log WHERE "; 
