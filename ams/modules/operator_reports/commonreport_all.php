...
cleardir("modules/operator_reports/images");
?>
...
$p->action="loadModule(1,'','operator_reports','CommonReportAll',\$H({search: 1}));"; $p->align2="right";
...
if ($duration1&&$duration2) $date_clause.=" AND ratesec >= $duration1 AND ratesec < $duration2";
    ...
    $QUERY = "SELECT COUNT(IF(verb = 'ENTERQUEUE',1,NULL)) AS all_calls,
		     ...
		     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 < 5,1,NULL)) AS less_five,
		     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 < 30,1,NULL)) AS less_thirty,
		     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 < 5,1,NULL)) AS asa1,
		     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 >= 5 AND data1 < 10,1,NULL)) AS asa2,
		     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 >= 10 AND data1 < 15,1,NULL)) AS asa3,
		     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 >= 15 AND data1 < 20,1,NULL)) AS asa4,
		     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 >= 20 AND data1 < 25,1,NULL)) AS asa5,
		     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 >= 25 AND data1 < 30,1,NULL)) AS asa6,
		     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 >= 30 AND data1 < 35,1,NULL)) AS asa7,
		     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 >= 35 AND data1 < 40,1,NULL)) AS asa8,
		     COUNT(IF(verb LIKE 'COMPLETE%' AND data1 >= 150,1,NULL)) AS asa9,
		     SUM(IF(verb LIKE 'COMPLETE%' ,data1,NULL)) AS asa10
...
if ($totalcall)include("modules/operator_reports/graph_calls.php");

?>
<br>

<IMG SRC="modules/operator_reports/images/<?=$filename_res?>" WIDTH="48%" ALIGN=RIGHT>
...
<table border="0" cellspacing="0" cellpadding="0" width="50%" class="report-tbl">
 <tr><td>
	...
	<tr >
		...
		<td align="center" nowrap><? print $Wait2Min; ?>%</td>
		<td align="center" nowrap><?=$owertwo;?>(<?=round(100-(($totalcall-$owertwo)*100/$totalcall),2)."%"?>)</td>
		<tr >
		<td align="center" height=30 nowrap>Переведено</td>
		<td align="center" nowrap>-</td>
		<td align="center" nowrap><?=$list_month[0][5];?>(<?=round(100-(($totalcall-$list_month[0][5])*100/$totalcall),2)."%"?>)</td>
		</tr>
		<tr>
		<td align="center" height=30 nowrap>Звонки с ожиданием на линии менее 5 секунд</td>
		<td align="center" nowrap>-</td>
		<td align="center" nowrap><?=$lessfive;?>(<?=round(100-(($totalcall-$lessfive)*100/$totalcall),2)."%"?>)</td>

		</tr>
		<tr>
		<td align="center" height=30 nowrap>Service Level (SL)</td>
		<td align="center" nowrap><?=$ServiceLevel?></td>
		<td align="center" nowrap>(<?=round($lessthirty/$totalcall*100,2)."%"?>)</td>
...
<?
  ...
  if ($dateperiod<20){
    ...
    include("modules/operator_reports/graph_day.php");
 ?>
  <br><br>

<h1 align="center">Принятые и не принятые звонки во временном разрезе</h1>

  <IMG SRC="modules/operator_reports/images/<?=$filenam_res?>" WIDTH="78%" ALIGN=RIGHT>
...
<?
    ...
    include ("modules/operator_reports/loadagent.php");
   if ($dateperiod<20){
    ...
    $name_agents = array();
       for($i=0; $i<$num; $i++){
        ...
        $aname = $list_agents[$i][5];
       	...
       	$w = workTime($start_time,$end_time,$aname);
       	$list_agents[$i][1] = $w['work_time'];
       	$list_agents[$i]['pause'] = $w['pause_time'];
       	//print '<br><br>';
       	$aname = 0;
       }
    $agents_compare = $num;
    ...
    include("modules/operator_reports/graph_agents.php");
?>
<br><br>
    ...
    <tr><td width="50%">
    <!--IMG SRC="modules/operator_reports/images/<?=$agents_res?>" WIDTH="47%" ALIGN=RIGHT-->
	...
	<td align="center" >Общее время разговора</td>
      ...
      <tr >
		...
		<td align="center" nowrap><?=display_hours($list_agents[$i][pause])?></td>
 ...
 <IMG SRC="modules/operator_reports/images/<?=$agents_res?>" width="100%" ALIGN=RIGHT>
 </td></tr>
 </table>
 </td>
 </tr></table>
...
<?
      ...
      include("modules/operator_reports/graph_asa.php");
  
?>
  <br>
  
    <h1 align="center">Средняя скорость ответа</h1>
    <table border="0" cellspacing="0" cellpadding="0" width="100%" class="report-tabl" style="background-color:white;">
    <tr><td width="20%">
	...
	<tr id="head">
	    ...
	    <td align="center" >Количество звонков</td>
	</tr>
	<tr>
	    <td align="center" >0-5</td>
	    <td align="center" ><?=$asa1?></td>
	</tr>
	<tr>
	    <td align="center" >5-10</td>
	    <td align="center" ><?=$asa2?></td>
	</tr>
	<tr>
	    <td align="center" >10-15</td>
	    <td align="center" ><?=$asa3?></td>
	</tr>
	<tr>
	    <td align="center" >15-20</td>
	    <td align="center" ><?=$asa4?></td>
	</tr>
	<tr>
	    <td align="center" >20-25</td>
	    <td align="center" ><?=$asa5?></td>
	</tr>
	<tr>
	    <td align="center" >25-30</td>
	    <td align="center" ><?=$asa6?></td>
	</tr>
	<tr>
	    <td align="center" >30-35</td>
	    <td align="center" ><?=$asa7?></td>
	</tr>
	<tr>
	    <td align="center" >35-40</td>
	    <td align="center" ><?=$asa8?></td>
	</tr>
	<tr>
	    <td align="center" > >150</td>
	    <td align="center" ><?=$asa9?></td>
	</tr>
	<tr>
	    <td align="center" >ASA</td>
	    <td align="center" ><?=round($asa10/$asa11,2)?></td>
	</tr>
  ...
  <td width="50%">
      <IMG SRC="modules/operator_reports/images/<?=$filenam_res?>" WIDTH="100%" ALIGN=RIGHT>
  </td>
