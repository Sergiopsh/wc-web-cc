...
include_once("modules/autocall/Userwithsumm.php");
...
list.deletelist=function (id,name) {
 if (!confirm("Удалить список\n" + name + "?")) return;
    loadModule('','autocall','autocall','deletelistwithsumm',$H({id: id})   );
}
...
list.viewlist=function (id) {
    loadModule('','autocall','autocall','viewlistwithsumm',$H({id: id})   );
}
list.SendFile=function() {
    ...
    $$f({
        ...
        url:'modules/autocall/uploadabonlistwithsumm.php',//адрес на серверный скрипт который будет принимать файл
