...
function GetOperNameByID($id){
    if(!$db) $db=DbConnect();
    $query = "select first_name FROM users where user_name like '%$id%'";
...
echo "</form>";
   ...
   $date_clause.=$sc;
    if ($iOperName==0){
	$query = "SELECT FROM_UNIXTIME(time_id-data1-data2),call_id,agent,verb,data1,data2,data3,data4
			FROM queuemetrics.queue_log
			WHERE (verb LIKE 'COMPLETE%' OR verb='TRANSFER' OR verb='ABANDON')
			AND queue in ('580','581') AND ";
    }else{
	$query = "SELECT FROM_UNIXTIME(time_id-data1-data2),call_id,agent,verb,data1,data2,data3,data4
			FROM queuemetrics.queue_log
			WHERE (verb LIKE 'COMPLETE%' OR verb='TRANSFER' OR verb='ABANDON')
			AND queue in ('580','581')
			AND agent like '%$iOperName%' AND
			";    
    };
   $query.=$date_clause;
...
$query = "SELECT call_id,data2 FROM queuemetrics.queue_log
			WHERE verb = 'ENTERQUEUE' AND queue in ('580','581') AND ";
if(!empty($iPhoneNumber))$query.= "data2 like '%$iPhoneNumber%' AND ";
///////////////////////////////////////////////
...
for ($i=0;$i<count($list_one);$i++){
	...
	$CsvData = $CsvData.GetOperNameByID(substr($data3,6,3)).$CsvDelim;
	...
	?>
	<td nowrap><a ></a></td>
	<td width=6><a title="<?=$strDownloadFile?>" href="#null" ONCLICK="window.open('http://10.10.11.34/records/search.php?&find=<?=$list_one[$cn][1];?>','','Toolbar=1,Location=0,Directories=0,Status=0,Menubar=0,Scrollbars=0,Resizable=0,Width=550,Height=400');" ><img src="images/download.gif" ></a></td>
	...
	<td align="center" nowrap><?=GetOperNameByID(substr($data3,6,3));?></td>
