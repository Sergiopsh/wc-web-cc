...
if(!$db) $db=DbConnect();
$QUERY="select id_list,list_name from autocall.abon_list_with_summ";
...
$p->action="loadModule(1,'autocall','autocall','TasksListWithSumm',\$H({search: 1}));"; $p->align2="left";
$tbl->show();
...
if (isset($submit_button)){
    $QUERY = "
	insert into autocall.tasks_with_summ(
	    task_name,
	    start_date,
	    end_date,
	    list_id,
...
task.deletetask=function (id,name) {
    ...
    loadModule('','autocall','autocall','deletetaskWithSumm',$H({id: id})   );
}

task.start_immediatly=function (id,name) {
    if (!confirm("Запустить задачу\n" + name + "?")) return;
    //if (!confirm("функция не поддерживается")) return;
    loadModule('','autocall','autocall','TaskWorkWithSumm',$H({id: id})   );
}
...
task.stop=function (id,name) {
    ...
    loadModule('','autocall','autocall','TaskStopWithSumm',$H({id: id})   );
}

task.pause=function (id,name) {
    if (!confirm("Поставить задачу\n" + name + " на паузу?")) return;
    loadModule('','autocall','autocall','TaskPauseWithSumm',$H({id: id})   );
}

task.viewtask=function (id,name) {
    //if (!confirm("Удалить список\n" + name + "?")) return;
    //if (!confirm("функция не поддерживается")) return;
    loadModule('','autocall','autocall','ViewTaskWithSumm',$H({id: id})   );
}
...
if(!$db) $db=DbConnect();
$list_tasks = $db->get_cond_list("","autocall.tasks_with_summ","","id,task_name,start_date,end_date,list_id,file_id,calls_in_min,task_status,try_delay,answer_wait,try_count",$current_page,$limit_display);
...
$tbl->tblHead(array("","5","","","","","","","","","","","","","","","",""),array("","","","Статус","Имя задания","Начать","Завершить","Список"," Запись","Звонков в мин.","Пауза,сек"," Ждать ответа,сек","Попыток","Обработанные","Необработанные","Успешные","Неуспешные"),"100%");
	...
	$query = "select list_name from autocall.abon_list_with_summ where id_list=".$list_tasks[$j][4];
	...
	$res = $db->get_list($query);
	  ...
	  $QUERY2 = "select sum(processed_by != 0) as proc,count(*)-sum(processed_by != 0) as non_proc,sum(call_status=2) as good,sum(call_status=4) as bad from autocall.calls_with_summ where task_id=".$list_tasks[$j][0]." limit 10";
