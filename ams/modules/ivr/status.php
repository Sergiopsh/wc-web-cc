...
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="report-tbl">
 <tr><td>
	...
	<td align="center">Действие: Встать на паузу</td>
	<td align="center">Действие: Выйти с паузы</td>
...
foreach($queues as $queue){
	...
	if (agent_in_queue($agent_queues,$queue)){ 
	    ...
	    if(get_agent_pause($queue_info_arr,$agent)){
		echo "<td align=\"center\">Нет действия";
		echo "<td align=\"center\"> <input class=\"sbutton\" type=\"button\" value=\"С паузы\" onclick=\"loadModule('','operator_queue','operator_queue','OutPause',\$H({agent: '$agent'}));\"> </td>";
	    }else{
		echo "<td align=\"center\"> <input class=\"sbutton\" type=\"button\" value=\"На паузу\" onclick=\"loadModule('','operator_queue','operator_queue','InPause',\$H({agent: '$agent'}));\"> </td>";
		echo "<td align=\"center\">Нет действия";
	    }
	}else{ 
