<?
  ...
  $sql.=$db->sql_time;
	...
	/************************/
	$QUERY = "SELECT department, SUM(ratesec),  
	...
	$QUERY.=$cur_query;
	$QUERY.=" FROM $table WHERE 1".$sql." GROUP BY department ORDER BY department";


	$list_bydeps = $db->get_list($QUERY);
	$num = $db->count;
...
if (!$num){ echo "<br><center><div class='module-note'>$strNoCalls</div></center>"; exit(); }
...
foreach ($list_bydeps as $data) if ($mmax < $data[1]) $mmax=$data[1];
/*
?>
<table border=0 width="98%">
<tr><td align="right" width="100%">
<div class='module-link'>
<a href="#" onclick="rep.printReport();" title="<?=$strPrintReport?>">
<img src="images/print.gif" align="absmiddle" title="<?=$strPrintReport?>">
<?=$strPrint?>
<a href="#" onclick="rep.exportPDF();" title="<?=$strExportPDF?>">
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="report-tbl">
 <tr><td>			
	...
	<tr id="head">
	<td align="left">&nbsp;<?=$strDepartment?></td>
	...
	foreach ($list_bydeps as $data){	
		...
		if ($mmax) $widthbar= intval(($data[1]/$mmax)*110);
		else $widthbar=0;
		
		if($list_cur) {
    		    $i=5;
    		    if($val) {
			unset($total_bydeps_sum);
    			foreach($list_cur as $c) {	
			    if($val == $c[8]) $base_course=$c[6];
			    if($c[6]) $total_bydeps_sum+=$data[$i]/$c[6]; //sum in USD
			    $i++;	
    			}
    			$total_bydeps_sum=$total_bydeps_sum*$base_course;
			$total_sum+=$total_bydeps_sum;
		    }else{
			unset($total_bydeps_sum_val);
    			foreach($list_cur as $c) {	
			    if($c[6]) $total_bydeps_sum_val[$c[8]]+=$data[$i]; //sum in USD
			    $total_sum_val[$c[8]]+=$total_bydeps_sum_val[$c[8]];
			    $i++;	
    			}
			
		    }
		} else $total_bydeps_sum=0;
	...
	?>
		...
		<?/*
		<td align="right" nowrap><? echo number_format($pdd,1);?></td>
		*/
		if($val) {?>
		<td align="center" nowrap><?=number_format($total_bydeps_sum,2)?></td>
		
		<?}else{
		    foreach($list_cur as $c) {?>
		      <td align="center" nowrap><?=number_format($total_bydeps_sum_val[$c[8]],2)?></td>
		    <?}
                  }		
