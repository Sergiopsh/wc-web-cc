<?
  ...
  if($t_accountcode) $sql.=" AND (t_accountcode = ".quote($t_acccountcode).")";
  ...
  $sql.=$db->sql_time;
	...
	/************************/
	$QUERY = "SELECT t_name, SUM(ratesec), count(*) , 
	count(IF(disposition = 'ANSWERED',1,NULL)), 
	...
	$QUERY.=$cur_query;
	$QUERY.=" FROM $table WHERE 1".$sql." GROUP BY accountcode ORDER BY accountcode";  

	$list_bytplans = $db->get_list($QUERY);
	$num = $db->count;
...
$totalcallanswered=0;
$totalpdd=0;
foreach ($list_bytplans as $data) if ($mmax < $data[1]) $mmax=$data[1];
/*
?>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="report-tbl">
 <tr><td>			
	...
	<tr id="head">
	<td align="left" widyj="15%">&nbsp;<?=$strFullTariffPlan?></td>
	...
	foreach ($list_bytplans as $data){	
		...
		if($list_cur) {
    		    $i=5;
    		    if($val) {
			unset($total_bytplan_sum);
    			foreach($list_cur as $c) {	
			    if($val == $c[8]) $base_course=$c[6];
			    if($c[6]) $total_bytplan_sum+=$data[$i]/$c[6]; //sum in USD
			    $i++;	
    			}
    			$total_bytplan_sum=$total_bytplan_sum*$base_course;
			$total_sum+=$total_bytplan_sum;
		    }else{
			unset($total_bytplan_sum_val);
    			foreach($list_cur as $c) {	
			    if($c[6]) $total_bytplan_sum_val[$c[8]]+=$data[$i]; //sum in USD
			    $total_sum_val[$c[8]]+=$total_bytplan_sum_val[$c[8]];
			    $i++;	
    			}
			
		    }
		} else $total_bytplan_sum=0;
	...
	?>
		...
		*/
		if($val) {?>
		<td align="center" nowrap><?=number_format($total_bytplan_sum,2)?></td>
		
		<?}else{
		    foreach($list_cur as $c) {?>
		      <td align="center" nowrap><?=number_format($total_bytplan_sum_val[$c[8]],2)?></td>
		    <?}
                  }		
