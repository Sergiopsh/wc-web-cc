...
class DT_Element {
	...
	$this->label=$label;
	$this->val=$val;
	if($style) $this->name.=" style=\"$style\" ";
	...
	echo "<td".$al1.$val1.$w1." nowrap=\"nowrap\">";
	if($required) echo "<font color=\"red\">*</font>&nbsp;";
	else echo "&nbsp;&nbsp;";
	echo htmlspecialchars($this->label)."</td>";
	echo "<td".$al2.$val2.$cs.$rs.$w2." nowrap=\"nowrap\">";    
...
class DT_Simple extends DT_Element {
	...
	echo "<td".$al.$cs.$rs.$w.">&nbsp;&nbsp;".htmlspecialchars($this->val)."</td>";
    }
...
class DT_Button extends DT_Element {
    ...
    var $submit = true;
    function setOptions($action,$submit=true) {
	$this->action=$action;
	$this->submit=$submit;
    }
    function show () {
	...
	if($this->submit) {
		...
		$this->action="if(!ams.checkInputs($req,$filters)) return false;".$this->action;
		//echo $this->action;
	    }
	}
	if($this->align2) $al=" align=\"".$this->align2."\"";	
	...
	echo "<td".$cs.$al.$val.$w."><input type=\"button\"".$this->name."onclick=\"".$this->action."\" value=\"".$this->val."\" $cls /></td>";
    }
...
class DT_Checkbox extends DT_Element {
	...
	echo "<input type=\"checkbox\"".$this->name."$check $cls /></td>";
    }
}
